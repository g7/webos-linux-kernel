...
static bool video0_has_data = false;
static bool video1_has_data = false;
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	if ((pdev->id == 0) && (pdev->num_resources > 0)) {
		...
		MSM_FB_INFO("msm_fb_probe:  fb0 phy_Addr = 0x%x virt = 0x%x\n",
			     (int)fbram_phys, (int)fbram);
		MSM_FB_INFO("msm_fb_probe:  fb1 phy_Addr = 0x%x virt = 0x%x\n",
			     (int)fb1ram_phys, (int)fb1ram);
	...
	if(0 != rc)
		printk("msmfb: state sysfs cannot be created!\n");
	...
	switch (mfd->enabled_fbs) {
		...
		case (LAYER_VIDEO_1|LAYER_FB1|LAYER_FB0):
			...
			if(mode == LAYER_FB1 && !on) {
				...
				//When video 0 is enabled, video 1 layer needs to be pushed down.
				//The following two calls ensure proper restructing.
				mdp4_overlay_update_lcd(mfd);
				if(video1_has_data) {
...
already_done:
	...
	printk("msm_fb: invalid to switch layer %s %s\n",
	...
	struct mdp_blit_int_req *req, int layer)
{
	...
	if(!req) {
		printk(KERN_ERR"%s: Invalid mdp_blit_int_req\n",__func__);
		return -1;
	}
	...
	if(layer == MSMV4L2_LAYER1) {
		...
		if(mfd->enabled_fbs & LAYER_VIDEO_0) {
	...
	MSM_FB_DEBUG("src h %d w %d\n",
		pipe->src_height, pipe->src_width);
	MSM_FB_DEBUG("src rect: y %d x %d h %d w %d\n",
		pipe->src_y, pipe->src_x,
		pipe->src_h, pipe->src_w);
	MSM_FB_DEBUG("dst rect: y %d x %d h %d w %d\n",
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	for(i=0; i<2; i++) {
		...
		if(i == 0) {
			...
			/* Take the physical framebuffer size from the board file */
			fix->smem_len = fbram_size;
...
static int msm_fb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	/*
	 * To satisfy the alignment restrictions by adreno gpu, the xoffset must
	 * be used, and the xres_virtual must be adjusted. This is OK except that
	 * the check below implies the new dimension introduced by xres_virtual
	 * must be backed by increasing physical memory. This is not an option.
	 * In reality, the extra memory is also not needed as the gpu could write
	 * into a continguous memory region.
	 *
	 * So at the moment, we are temporarily re-defining the meaning of
	 * xres_virtual to allow xoffset to be configured more freely to suit
	 * adreno gpu's alignment requirements.
	 *
	 * The longer term investigation is to find a way to configure
	 * xoffset without making compromise on the xres_virtual definition.
	 *
	 */
#if 0
